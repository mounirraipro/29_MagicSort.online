...
//game text display
var textDisplay = {
    selectTube: "CHOOSE A TUBE",
    selectLevel: "PICK A LEVEL",
    level: "STAGE [NUMBER]",
    clear: "STAGE [NUMBER] CLEAR",
    score: "[NUMBER] PTS",
    timesup: "OUT OF TIME",
    exitTitle: 'LEAVE MAGIC SORT',
...
function saveLevelData() {
    if (Number(gameData.levelNum) >= gameData.levelCompleted) {
        ...
        gameData.levelCompleted = gameData.levelCompleted >= levelSettings.length ? levelSettings.length : gameData.levelCompleted;
        Cookies.set(cookieName, gameData.levelCompleted, {
            expires: 360
        });
    }
}
...
function initHTMLInterface() {
    if (window.htmlInterfaceReady) {
        return;
    }
    ...
    $('#htmlLevelNext').on('click', function() {
        playSound('soundButton');
        toggleSelect(true);
    });
    ...
    $('#htmlTubeNext').on('click', function() {
        playSound('soundMatch');
        toggleTube(true);
    });

    $('#htmlTubePlay').on('click', function() {
        playSound('soundButton');
        goPage('game');
    });

    $('#htmlResultContinue').on('click', function() {
        playSound('soundButton');
        if (gameData.type == 'level') {
            goPage('level');
        } else {
    ...
    $('#htmlConfirmLeave').on('click', function() {
        playSound('soundButton');
        togglePop(false);
        stopAudio();
        stopGame();
    ...
    $('#htmlConfirmStay').on('click', function() {
        playSound('soundButton');
        togglePop(false);
    });

    $('#htmlSettingsToggle').on('click', function() {
        toggleOption();
    });

    $('#htmlSoundButton').on('click', function() {
        toggleSoundMute(!buttonSoundOn.visible);
    });

    $('#htmlMusicButton').on('click', function() {
...
function updateHTMLConfirm() {
    $('#htmlConfirmTitle').text(textDisplay.exitTitle);
    $('#htmlConfirmText').html(textDisplay.exitMessage.replace(/\n/g, '<br>'));
    $('#htmlConfirmMenu').toggleClass('is-hidden', !confirmContainer.visible);
}
...
function unlockLevelTween(r, c) {
    ...
    TweenMax.to($.level[r + '_unlock_' + c], .2, {
        delay: .5,
        alpha: 1,
        scaleX: 1.2,
        scaleY: 1.2,
...
function resizeGameUI() {
    ...
    if (viewport.isLandscape) {
        ...
        buttonLevelArrowL.x = -210;
        buttonLevelArrowR.x = 210;
        ...
        selectLevelTitleTxt.y = -(canvasH / 100 * 25);
        buttonLevelArrowL.x = -165;
        buttonLevelArrowR.x = 165;
        buttonLevelArrowL.y = 0;
        buttonLevelArrowR.y = 0;
...
function createTube(x, y) {
    ...
    newLiquidContainer.data = {
        active: true,
        index: gameData.tubes.length,
        x: x,
        y: y,
        fill: 0,
        fillShape: null,
...
function moveTube(pourIndex) {
    ...
    TweenMax.to(thisLiquid, gameSettings.moveSpeed, {
        x: pos.x,
        y: pos.y,
    ...
    TweenMax.to(thisLiquid.data.mask, gameSettings.moveSpeed, {
        rotation: thisLiquid.data.rotateNum,
        ease: Linear.easeNone,
        overwrite: true
    });
    TweenMax.to(thisLiquid.data.imageBack, gameSettings.moveSpeed, {
        rotation: thisLiquid.data.rotateNum,
        ease: Linear.easeNone,
        overwrite: true
    });
    TweenMax.to(thisLiquid.data.imageFront, gameSettings.moveSpeed, {
        rotation: thisLiquid.data.rotateNum,
        ease: Linear.easeNone,
        overwrite: true
    });
    TweenMax.to(thisLiquid.data.container, gameSettings.moveSpeed, {
        y: thisLiquid.data.containerY,
        ease: Linear.easeNone,
        overwrite: true
    });
}
...
function pourLiquid(pourIndex) {
    ...
    TweenMax.to(thisShape, gameSettings.fillSpeed, {
        fillH: 0,
        overwrite: true,
        ease: Linear.easeNone,
        onUpdate: function() {
    ...
    if (con) {
        if (autoFill) {
            ...
            for (var n = 0; n < leftTubesArr.length; n++) {
                ...
                TweenMax.to(thisTube, delayCount, {
                    overwrite: true,
                    onComplete: actionTube,
                    onCompleteParams: [thisTube]
                });
                delayCount += delayNum;
            }
        }
...
function animateBubble(newBubble, y) {
    ...
    TweenMax.to(newBubble, tweenSpeed, {
        delay: delayNum,
        y: y,
        onComplete: function() {
...
function animateTimer() {
    var tweenSpeed = .8;
    timerShape.alpha = .2;
    TweenMax.to(timerShape, tweenSpeed, {
        alpha: 1
    });
    timerShapeBg.alpha = 0;
    TweenMax.to(timerShapeBg, tweenSpeed, {
        alpha: .3
    });
}
